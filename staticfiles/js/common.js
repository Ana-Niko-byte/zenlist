...
// export function amendFormat(retrievedArray){
//     /**
...
//         let date = item.getAttribute("data-reviewedOn");
...
export function amendFormat(retrievedArray, dataAttribute){
    /**
     * Iterates over each item in the array and retrieves the dataset reviewedOn value
     * representing the date on which the review was made.
     * 
    ...
    for (const item of retrievedArray){
        ...
        item.dataset.taskDueDate = `${year}-${zeroedM}-${zeroedD}`;
    };
};

export function returnRecent(arrayToDuplicate, datasetValue, taskBoxId, amendDataValue){
    /**
     * 
     */
    const newArray = [...arrayToDuplicate];
    amendFormat(newArray, amendDataValue);
...
export function TaskSort(mainSelect, accordionIdentifier, taskBoxId){
    document.getElementById(mainSelect).addEventListener("change", function(){
        ...
        if (selectedValue === 'by priority'){
            ...
            returnRecent(taskArray, 'taskDueDate', taskBoxId, "data-taskDueDate");
        }
    });
};